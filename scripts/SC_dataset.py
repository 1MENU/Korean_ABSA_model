...
def SC_dataset(raw_data, tokenizer, max_len):
    ...
    polarity_e1_mask_list = []
    polarity_e2_mask_list = []
    ...
    for utterance in raw_data:
        ...
        polarity_e1_mask_list.extend(polarity_data_dict['e1_mask'])
        polarity_e2_mask_list.extend(polarity_data_dict['e2_mask'])
    ...
    return TensorDataset(
        ...
        torch.tensor(e2_mask_list)
            ...
            torch.tensor(polarity_e1_mask_list),
            torch.tensor(polarity_e2_mask_list)
        )
    ...
    for pair in entity_property_pair:
        ...
        for annotation in annotations:
            ...
            if entity_property == pair:
                ...
                polarity_data_dict['e1_mask'].append(e1_mask)
                polarity_data_dict['e2_mask'].append(e2_mask)
...
def get_SC_dataset(train_data, dev_data, test_data, pretrained_tokenizer, max_len):
    tokenizer = AutoTokenizer.from_pretrained(pretrained_tokenizer)
    num_added_toks = tokenizer.add_special_tokens(special_tokens_dict)
    
    train_CD_data, train_SC_data = SC_dataset(train_data, tokenizer, max_len)
    dev_CD_data, dev_SC_data = SC_dataset(dev_data, tokenizer, max_len)
    test_CD_data, test_SC_data = SC_dataset(test_data, tokenizer, max_len)
